...
legend({"Low $b$","Med $b$","High $b$"},'interpreter','latex','location','northwest')
xlabel('Tax Param $\kappa$','Interpreter','latex');
ylabel('Rounds','Interpreter','latex');
title('\textbf{Rounds to $\kappa$}','Interpreter','latex');
grid on
set(gcf, 'Position', [100, 500, 800, 700]);  % Set figure size in pixels
set(gca, 'FontSize', 23, 'FontName','times new roman');
exportgraphics(gca,'../Plots/rounds.pdf', 'Resolution',300);
...
ylabel("Normalized system cost",'Interpreter','latex')
title("\textbf{Normalized system cost versus $\kappa$}",'Interpreter','latex')
grid on
set(gcf, 'Position', [1000, 500, 800, 700]);  % Set figure size in pixels
set(gca, 'FontSize', 23, 'FontName','times new roman');
exportgraphics(gca,'../Plots/costPerk.pdf', 'Resolution',300);
...
for i = 1:dataLen
    ...
    roundBound(i) = ceil((data.taxParam(i)) * Bmax(i) * 10) ;
end
...
loglog(1:30, 1:30, 'k')
legend({"Low $b$","Med $b$","High $b$"},'interpreter','latex')
xlabel('Round bound: $\big\lceil \kappa B_{\max} \big\rceil$','Interpreter','latex');
ylabel('Rounds: $r_{\mathbf{term}}$','Interpreter','latex');
grid on
set(gca,'fontsize',23,'FontWeight','normal')
set(gca, 'FontName', 'Times New Roman', 'TickLabelInterpreter','latex','FontWeight','normal');
set(gcf, 'Position', [1000, 100, 800, 700]);  % Set figure size in pixels
set(gca,'LineWidth',1.5)
exportgraphics(gca,'../Plots/roundBound.pdf', 'Resolution',300);
...
legend({"Low $b$","Med $b$","High $b$"},'interpreter','latex')
xlabel('Efficiency bound: $n \left( B_{\max}\,\delta_r + \frac{B_{\max}}{\alpha_r} \right)$','Interpreter','latex');
...
set(gca,'LineWidth',1.5)
exportgraphics(gca,'../Plots/gapBound.pdf', 'Resolution',300);
...
grid on
set(gca,'fontsize',23,'FontWeight','normal')
set(gca, 'FontName', 'Times New Roman', 'TickLabelInterpreter','latex');
set(gcf, 'Position', [100, 100, 800, 700]);  % Set figure size in pixels
set(gca,'LineWidth',1.5)
ylim([0 5.2])
exportgraphics(gca,'../Plots/CostComparison.pdf', 'Resolution',300);
...
set(gca,'fontsize',23,'FontWeight','normal')
set(gca, 'FontName', 'Times New Roman', 'TickLabelInterpreter','latex','FontWeight','normal');
set(gcf, 'Position', [500, 100, 800, 700]);  % Set figure size in pixels
set(gca,'LineWidth',1.5)
exportgraphics(gca,'../Plots/GiniComparison.pdf', 'Resolution',300);

%% Define function
function out = ComputeBMax(data, idx)
    l = length(data.negoOut_history{idx});
    out = 0;
    for j = 1:l
        C = data.negoOut_history{idx}{j}.C;
